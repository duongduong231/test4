...
module.exports.run = async function({ api, event, args, Currencies, utils }) {
	if (!args[0]) {
		const money = (await Currencies.getData(`${(event.type == "message_reply") ? event.messageReply.senderID : event.senderID}`)).money;
		return api.sendMessage(`Số tiền ${(event.type == "message_reply") ? event.messageReply.senderID : "bạn"} hiện đang có là: ${money} LE`, event.threadID);
	}
	else if (Object.keys(event.mentions).length == 1) {
		var mention = Object.keys(event.mentions)[0];
		const money = (await Currencies.getData(mention)).money;
		return api.sendMessage({
			body: `Số tiền của ${event.mentions[mention].replace("@", "")} hiện đang có là: ${money} LE.`,
			mentions: [{
		...
		for (let value of mention) {
			...
			msg += (` - ${event.mentions[value].replace("@", "")}: ${data.money} LE\n`);
		};
