  	...
  	var { threadID, messageID, senderID } = event
        ...
        if (moneydb == undefined) return api.sendMessage("Người dùng chưa tồn tại!", event.threadID, event.messageID);
    		...
    		(money == "all") ? ((moneydb > 10000) ? money = 10000 : money = moneydb) : "";
    			...
    			if (money > 100000) return api.sendMessage(`[ ROUL ] Số tiền đặt cược của bạn quá khủng, tối đa là 100000 LE!`, event.threadID, event.messageID);
				if (money < 200) return api.sendMessage(`[ ROUL ] Số tiền đặt cược của bạn quá nhỏ, tối thiểu là 200 LE`, threadID, messageID);
        ...
        if (color == 0 && check(random) == '😡') api.sendMessage(`[ ROUL ]\nBạn đã chọn màu 😡, bạn đã thắng với số tiền được nhân lên 4: ${money * 4} LE\nSố tiền hiện tại của bạn là: ${moneydb + (money * 4)} LE.`, threadID, () => Currencies.increaseMoney(senderID, parseInt(money * 4)), messageID);
				else if (color == 1 && check(random) == '♥️') api.sendMessage(`[ ROUL ]\nBạn đã chọn màu ♥️, bạn đã thắng với số tiền nhân lên 2: ${money * 2} LE\nSố tiền hiện tại của bạn là: ${moneydb + (money * 2)} LE.`, threadID, () => Currencies.increaseMoney(senderID, parseInt(money * 2)), messageID);
				else if (color == 2 && check(random) == '💚') api.sendMessage(`[ ROUL ]\nBạn đã chọn màu 💚, bạn đã thắng với số tiền nhân lên 1: ${money * 1} LE\nSố tiền hiện tại của bạn là: ${moneydb + (money * 1)} LE.`, threadID, () => Currencies.increaseMoney(senderID, parseInt(money * 1)), messageID);
				else if (color == 3 && check(random) == '💛') api.sendMessage(`[ ROUL ]\nBạn đã chọn màu 💛, bạn đã thắng với số tiền nhân lên 0.75: ${money * 0.75} LE\nSố tiền hiện tại của bạn là: ${moneydb + (money * 0.75)} LE.`, threadID, () => Currencies.increaseMoney(senderID, parseInt(money * 0.75)), messageID);
				else if (color == 4 && check(random) == '💜') api.sendMessage(`[ ROUL ]\nBạn đã chọn màu 💜, bạn đã thắng với số tiền nhân lên 0.5: ${money *0.5} LE\nSố tiền hiện tại của bạn là: ${moneydb + money *0.5} LE.`, threadID, () => Currencies.increaseMoney(senderID, parseInt(money *0.5)), messageID);
				else if (color == 5 && check(random) == '🖤️') api.sendMessage(`[ ROUL ]\nBạn đã chọn màu 🖤️, bạn đã thắng với số tiền nhân lên 0.1: ${money * 0.1} LE\nSố tiền hiện tại của bạn là: ${moneydb + (money * 0.1)} LE.`, threadID, () => Currencies.increaseMoney(senderID, parseInt(money * 0.1)), messageID);
				else api.sendMessage(`[ ROUL ]\nMàu ${check(random)}\nBạn đã ra đê ở và mất trắng số tiền: ${money} LE :'(\nSố tiền hiện tại của bạn là: ${moneydb - money} LE.`, threadID, () => Currencies.decreaseMoney(senderID,  parseInt(money)), messageID);
}
