...
module.exports.event =async function({ api, event }) {
	...
	if (event.type !== "message_unsend" && event.body.length !== -1) {
        ...
        let userName = info[event.senderID].name, gender = info[event.senderID].gender == 2 ? "Anh" : info[event.senderID].gender == 1 ? "Chị" : "Con bóng" ;
				if (shortOut.indexOf(" | ") !== -1) {
					var arrayOut = shortOut.split(" | ");
					return api.sendMessage(`${arrayOut[Math.floor(Math.random() * arrayOut.length)].replace(/\{name}/g, userName)}`, event.threadID);
				}
				else return api.sendMessage(`${shortOut.replace(/\{name}/g, userName).replace(/\{gender}/g, gender)}`, event.threadID);
			}
		}
	}
}
...
module.exports.run = function({ api, event, args }) {
	...
	if (content.indexOf(`xóa`) == 0 || content.indexOf(`xoá`) == 0) {
		...
		return fs.readFile(__dirname + "/cache/shortcut.json", "utf-8", (err, data) => {
			...
			if (allData.some(item => item.id == threadID)) {
				...
				return fs.writeFile(__dirname + "/cache/shortcut.json", JSON.stringify(getThread), "utf-8", (err) => (err) ? console.error(err) : api.sendMessage("Đã xóa toàn bộ shortcut" , threadID, messageID));
			}
		});	
  else {
