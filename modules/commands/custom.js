...
module.exports.run = async ({ api, event, args, Threads, utils, client }) => {
    ...
    switch (args[0]) {
        case "joinMessage": {
            ...
            if (msg == "reset") {
            settings["customJoin"] = undefined;
            await Threads.setData(event.threadID, options = { settings });
            client.threadSetting.set(event.threadID, settings);
            return api.sendMessage("Đã gỡ bỏ thành công customJoin của nhóm bạn!", event.threadID);
            }
        ...
        case "leaveMessage": {
            ...
            return api.sendMessage("Đã gỡ bỏ thành công leaveMessage của nhóm bạn!", event.threadID);
            await Threads.setData(event.threadID, options = { settings });
            client.threadSetting.set(event.threadID, settings);
            }            
        ...
        case "rankupMessage": {
            let msg = args.slice(1, args.length).join(" ");
            if (msg == "reset") {
            settings["customRankup"] = undefined;
            return api.sendMessage("Đã gỡ bỏ thành công customRankup của nhóm bạn!", event.threadID);
            await Threads.setData(event.threadID, options = { settings });
            client.threadSetting.set(event.threadID, settings);
            }       
