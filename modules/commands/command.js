...
module.exports.run = async ({ event, api, __GLOBAL, client, args, utils }) => {
    ...
    var nocmdData = JSON.parse(readFileSync(__dirname + "/cache/cmds.json"));
    //tạo 1 đối tượng mới nếu group chưa có trong file cmds
    if (!nocmdData.banned.some(item => item.id == event.threadID)) {
      let addThread = {
        id: event.threadID,
        cmds: []
      };
      nocmdData.banned.push(addThread);
      writeFileSync(__dirname + "/cache/cmds.json", JSON.stringify(nocmdData));
  ...
  const content = args.slice(1, args.length);
	switch (args[0]) {
		...
		break;
		case "unban": {
			if (!args[1]) return api.sendMessage("không được để trống", event.threadID, event.messageID);
      var jsonData = JSON.parse(readFileSync(__dirname + "/cache/cmds.json"));
      var getCMDS = jsonData.banned.find(item => item.id == event.threadID).cmds;
      if (!getCMDS.find(a => a == args[1])) return api.sendMessage("Lệnh " + args[1] + " chưa bị cấm",event.threadID,event.messageID);
      else {
        let getIndex = getCMDS.indexOf(args[1]);
        getCMDS.splice(getIndex, 1);
        api.sendMessage("Đã bỏ cấm " + args[1] + " trong group này",event.threadID,event.messageID);
      }
      return writeFileSync(__dirname + "/cache/cmds.json",JSON.stringify(jsonData),"utf-8");
		}
		break;	
    case "ban": {
      if (!args[1]) return api.sendMessage("Hãy nhập lệnh cần cấm!", event.threadID, event.messageID);
      var jsonData = JSON.parse(fs.readFileSync(__dirname + "/cache/cmds.json"));
      if (args[1] == "list") return api.sendMessage(`Đây là danh sách các command hiện đang bị lock tại group này: ${nocmdData.banned.find(item => item.id == event.threadID).cmds}`,event.threadID, event.messageID);
      if (!jsonData.cmds.find(a => a == args[1])) return api.sendMessage("Không có lệnh " + args[1] + " trong cmds.json nên không thể cấm",event.threadID, event.messageID);
      else {
        if (jsonData.banned.some(item => item.id == event.threadID)) {
          let getThread = jsonData.banned.find(item => item.id == event.threadID);
          if (getThread.cmds.includes(args[1])) return api.sendMessage("Lệnh " + args[1] + " đã bị ban từ trước",event.threadID, event.messageID);
          getThread.cmds.push(args[1]);
        } else {
          let addThread = {
            id: event.threadID,
            cmds: []
          };
          addThread.cmds.push(args[1]);
          jsonData.banned.push(addThread);
        }
        api.sendMessage("Đã cấm " + args[1] + " trong group này",event.threadID, event.messageID);
      }
      return fs.writeFileSync(__dirname + "/cache/cmds.json",JSON.stringify(jsonData),"utf-8");
		}
		break;
    default:
