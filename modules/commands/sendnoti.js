...
module.exports.run = async ({ api, event, args, client, __GLOBAL }) => {
  const allThread = __GLOBAL.settings.allow
	...
	return api.sendMessage(`Đã gửi tin nhắn đến ${count} nhóm!`, event.threadID, () => api.sendMessage(`[!] Không thể gửi thông báo tới ${cantSend.length} nhóm: \n ${cantSend.join("\n")}`, event.threadID, event.messageID), event.messageID);
}