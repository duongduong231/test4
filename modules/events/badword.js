...
module.exports.run = async function({ api, event }) {
  const { messageID, threadID, senderID } = event;
	const fs = require("fs-extra");
  if (!fs.existsSync(__dirname + `/../commands/cache/badword.json`)) return;
  const threadInfo = await api.getThreadInfo(threadID);
  if (!threadInfo.adminIDs.find(el => el.id == api.getCurrentUserID())) return;
	const badword = JSON.parse(fs.readFileSync(__dirname + `/../commands/cache/badword.json`));
  ...
  let ids = [], nameArray = [], mentions= [];
		for (let i of event.logMessageData.addedParticipants) {
			let id = i.userFbId;
      let a = JSON.stringify(badword.banned)
