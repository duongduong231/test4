   ...
   if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    const fs = require("fs-extra");
      ...
      //if (!fs.existsSync(__dirname + `/../commands/cache/bans.json`)) return;
	    //var bans = JSON.parse(fs.readFileSync(__dirname + `/../commands/cache/bans.json`));				
      //setTimeout(() => {
     // if(bans.banned[event.threadID].includes(id)) return api.removeUserFromGroup(id, event.threadID);},2000)
